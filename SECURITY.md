...
`pdf-extract-text` runs entirely in your browser. There is no server component and no
account system, so your files are never uploaded. Most classic web vulnerabilities
...
- The text extraction pipeline, the service worker, and the PWA manifest.
- Anything that could send PDF data or metadata off the device.
